...
@server.rtc_session(agent_name="dev-outbound-caller")
async def entrypoint(ctx: JobContext):
    print(f"\n\n>>> ENTRYPOINT STARTED: JobID={ctx.job.id} <<<\n\n")
    ...
    try:
        if ctx.job.metadata:
            ...
            logger.warning("Job has NO metadata.")
    except Exception as e:
        logger.error(f"Failed to parse job metadata as JSON: {e}")
        # Fallback for raw string metadata
        phone_number = ctx.job.metadata
        logger.info(f"FALLBACK: Using raw metadata as phone number: {phone_number}")

    # session = AgentSession(
    #     stt=inference.STT(model="deepgram/nova-3", language="multi"),
    #     # stt=inference.STT(model="cartesia/ink-whisper", language="bn"), #cant do bengali at all
    #     # llm=inference.LLM(model="openai/gpt-4.1-nano"),
    #     llm=inference.LLM(
    #         model="google/gemini-3-flash",  # e2e 4000ms+
    #     ),
    #     # tts=inference.TTS(
    ...
    #     # ),
    #     # tts=inference.TTS(
    #     #     model="inworld/inworld-tts-1.5-mini",  #2000ms e2e # Best Voice for the Indian Accent
    #     #     voice="Riya",
    #     #     language="hi-IN"
    #     # ),
    #     tts=inference.TTS(
    #         model="xai/tts-1",
    #         voice="Ara",              # Ara, Eve, Rex, Sal, Leo #Best voice voiice for the money and can speek hindi and Bengali
    #         language="multi"
    #     ),
    ...
    if supabase and call_id:
        logger.info(f"UPDATING SUPABASE: Setting call {call_id} to 'in_progress'...")
        try:
            res = supabase.table('calls').update({"status": "in_progress"}).eq("id", call_id).execute()
            logger.info(f"SUPABASE UPDATE SUCCESS: {res}")
        except Exception as e:
            logger.error(f"SUPABASE UPDATE FAILED: {e}")
    else:
        logger.warning(f"SKIPPING STATUS UPDATE: Supabase client exists: {supabase is not None}, CallID exists: {call_id is not None}")
    ...
    # When running in console mode, JobContext is a mock and doesn't have wait_until_closing
    if call_id and hasattr(ctx, 'wait_until_closing'):
    ...
    if supabase and call_id:
        ...
        try:
            ...
            elif hasattr(session, 'chat_context'): # Fallback for different SDK versions
            ...
            duration = 0
            try:
                if hasattr(ctx.job, 'created_at') and ctx.job.created_at:
                    duration = int(time.time() - ctx.job.created_at.timestamp())
            except Exception:
                pass
            ...
            logger.info("Successfully updated Supabase with transcript and final status.")
        except Exception as e:
